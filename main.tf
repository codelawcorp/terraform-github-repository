...
locals {
  # default_branch = coalesce(try(github_branch_default.this[0].branch, null), try(data.github_repository.template[0].default_branch, null), var.default_branch)
...
resource "github_branch_default" "this" {
  ...
  # Must be true, so the default branch is not managed by github_branch resource.
  rename = true
}
